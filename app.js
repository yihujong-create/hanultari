...
window.doLogin=function(){
  ...
  // 첫 로그인(기본 비밀번호)이면 변경 안내
  if(pw==='0000' && (!DB.memberPw||!DB.memberPw[nm])){
    setTimeout(()=>{ if(confirm(`초기 비밀번호(0000)로 로그인했습니다.\n비밀번호를 변경하시겠습니까?`)) showChangePw(); },800);
  }
};
...
// ── 핸디 ────────────────────────────────────────
function getHcp(nm){ const m=DB.members.find(x=>x.nm===nm); return m?(m.handicap||0):0; }
...
function calcFine(member,valid){
  ...
  const s=DB.settings, av=member.aver;
...
function calcAwards(){
  const s=DB.settings;
  const present=DB.members.filter(m=>(DB.scores[m.nm]||[]).some(x=>x!==null&&x!==undefined));
  const stats=present.map(m=>{
    const sc=DB.scores[m.nm].filter(x=>x!==null&&x!==undefined);
    const total=sc.reduce((a,b)=>a+b,0);
    return {...m,total,games:sc.length,overAver:total-m.aver*sc.length,scores:sc};
  });
...
function renderDashboard(){
  ...
  sorted.forEach(m=>{
    ...
    const hcp=m.handicap||0;
    const fine=isAbs?{amt:0}:calcFine(m,valid);
    const fineSpan=(!isAbs&&fine.amt>0)?`<span style="font-size:10px;font-weight:700;color:var(--red);margin-left:3px">₩${fmt(fine.amt)}</span>`:'';
    ...
    scrollHtml+=`<div class="mr t${tCls}" ${isAbs?'style="opacity:.55"':''}>
      ...
      <div class="mr-info">
        <div style="display:flex;gap:3px;flex-wrap:wrap;align-items:center">
          <span class="gb ${gradeCls(m.group)}">${m.group}조</span>${roleH}
...
function scoreCard(m,gc){
  ...
  let html=`<div class="sc-card" ${isAbs?'style="opacity:.55"':''}>
    <div class="sc-hdr">
      <span class="sc-nm">${m.nm}</span>
      <span class="sc-team ${tCls}">${m.team?m.team+'팀':'팀없음'}</span>
      <span class="gb ${gradeCls(m.group)}">${m.group}조</span>${roleH}
      ${hcp>0?`<span style="font-size:8px;background:#1a0d2b;color:var(--pur);border-radius:3px;padding:1px 5px;font-weight:700">H+${hcp}</span>`:''}
  ...
  if(isAbs){
    ...
    html+=`</div><div class="sc-sum">
      ...
      <span>Aver <b style="color:var(--blu)">${m.aver}</b></span>
      <span style="color:${total-m.aver*valid.length>=0?'var(--grn)':'var(--red)'}">${total-m.aver*valid.length>=0?'+':''}${total-m.aver*valid.length}</span>
    </div>`;
...
function _refreshScoreSum(nm){
  ...
  const totalH=scH.reduce((a,b)=>a+b,0);
  const avgH=scH.length?Math.round(totalH/scH.length):0;
  const diff=total-m.aver*valid.length;
  // sc-sum 요소 찾아서 업데이트
  ...
  cards.forEach(card=>{
    if(card.querySelector('.sc-nm')?.textContent===nm){
      ...
      if(sum) sum.innerHTML=`
        ...
        ${hcp>0?`<span>핸디총점 <b style="color:var(--pur)">${totalH}</b></span><span>핸디avg <b style="color:var(--pur)">${avgH}</b></span>`:''}
        <span>Aver <b style="color:var(--blu)">${m.aver}</b></span>
        <span style="color:${diff>=0?'var(--grn)':'var(--red)'}">${diff>=0?'+':''}${diff}</span>`;
      // 벌금 재계산
...
window.archiveMeet=function(){
  ...
  const entry={
    ...
    // 회원 Aver·핸디 스냅샷
    memberSnap: DB.members.map(m=>({nm:m.nm,aver:m.aver,handicap:m.handicap||0,team:m.team,group:m.group,role:m.role}))
  };
  DB.members.forEach(m=>{entry.handicaps[m.nm]=m.handicap||0;});
  ...
  if(awardTab==='award'){
    ...
    if(s.features.allCover){
      html+=`<div class="aw-card" style="border-color:rgba(167,139,250,.35)">
        ...
        <div style="display:flex;gap:5px;flex-wrap:wrap">`;
      DB.members.filter(m=>(DB.scores[m.nm]||[]).some(x=>x!==null)).forEach(m=>{
    ...
    DB.members.forEach(m=>{
      ...
      html+=`<div class="fc">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;flex-wrap:wrap">
          ...
          <span class="gb ${gradeCls(m.group)}">${m.group}조</span>
          <span style="font-size:10px;color:var(--txt2)">Aver ${m.aver}</span>
...
let rankMode='total';
function renderRank(){
  const present=DB.members.filter(m=>(DB.scores[m.nm]||[]).some(x=>x!==null));
  const stats=present.map(m=>{
    const sc=DB.scores[m.nm].filter(x=>x!==null&&x!==undefined);
    const total=sc.reduce((a,b)=>a+b,0);
  ...
  const tStat={A:{t:0,n:0},B:{t:0,n:0},C:{t:0,n:0},D:{t:0,n:0}};
  stats.forEach(m=>{if(tStat[m.team]){tStat[m.team].t+=m.totalH;tStat[m.team].n+=m.scores.length;}});
  const activeTeams=['A','B','C','D'].filter(t=>tStat[t].n>0);
  ...
  stats.forEach((m,i)=>{
    ...
    html+=`<div class="rk">
      ...
      <div class="rk-info">
        <div style="display:flex;gap:3px;flex-wrap:wrap"><span class="gb ${gradeCls(m.group)}">${m.group}조</span>${roleH}<span style="font-size:9px;color:var(--txt2)">${m.team?m.team+'팀':'팀없음'}</span>${m.hcp>0?`<span style="font-size:8px;color:var(--pur)"> H+${m.hcp}</span>`:''}</div>
...
function histCurrent(){
  ...
  const sorted=[...DB.members].sort((a,b)=>b.aver-a.aver);
  ...
  sorted.forEach((m,i)=>{
    ...
    html+=`<div class="mr t${teamCls(m.team)}">
      ...
      <div class="mr-info">
        <div style="display:flex;gap:3px;flex-wrap:wrap"><span class="gb ${gradeCls(m.group)}">${m.group}조</span>${roleH}<span style="font-size:9px;color:var(--txt2)">${m.team?m.team+'팀':'팀없음'} | Aver ${m.aver}</span>${hcp>0?`<span style="font-size:8px;color:var(--pur)">H+${hcp}</span>`:''}</div>
  ...
  DB.members.forEach(m=>{
    ...
    if(!v.length) return;
    const t=v.reduce((a,b)=>a+b,0),av=Math.round(t/v.length),diff=av-m.aver;
    const tc=teamColor(m.team);
    html+=`<tr><td class="hnm">${m.nm}</td><td><span style="color:${tc};font-size:9px">${m.team||'-'}</span></td><td>${m.aver}</td>`;
    ...
    if(isEditing){
      ...
      html += `<div style="background:var(--s1);border:2px solid var(--blu);border-radius:12px;padding:12px;margin-bottom:2px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          ...
          <div>
            <label style="font-size:10px;color:var(--txt2);display:block;margin-bottom:3px">팀</label>
            <select id="ed_team" class="fs" style="padding:7px 9px;font-size:13px">
          ...
          <div>
            <label style="font-size:10px;color:var(--txt2);display:block;margin-bottom:3px">개인 Aver</label>
            <input id="ed_aver" class="fi" type="number" inputmode="numeric" style="padding:7px 9px;font-size:13px" value="${m.aver}" min="0" max="300">
          </div>
          <div>
            <label style="font-size:10px;color:var(--pur);display:block;margin-bottom:3px">핸디 (게임당)</label>
            <input id="ed_hcp" class="fi" type="number" inputmode="numeric" style="padding:7px 9px;font-size:13px;border-color:rgba(167,139,250,.5)" value="${m.handicap||0}" min="0" max="100">
          </div>
        </div>
...
window.saveMemberEdit = function(i){
  ...
  const team = document.getElementById('ed_team').value;
  const fee  = parseInt(document.getElementById('ed_fee').value) || 0;
  const aver = parseInt(document.getElementById('ed_aver').value) || 0;
  const hcp  = Math.max(0, parseInt(document.getElementById('ed_hcp').value) || 0);
...
window.addMember = function(){
  ...
  const team = prompt('팀 (A/B/C/D, 없으면 빈칸)', '');
  const aver = parseInt(prompt('개인 Aver', '180')) || 180;
  const hcp  = parseInt(prompt('핸디 점수 (없으면 0)', '0')) || 0;
  const role = prompt('역할 (회장/총무/경기이사, 없으면 빈칸)', '');
  const fee  = parseInt(prompt('월회비', '40000')) || 40000;
  const t = ['A','B','C','D'].includes((team||'').toUpperCase()) ? (team||'').toUpperCase() : '';
  DB.members.push({nm, team:t, role, fee, aver, group:calcGrade(aver), handicap:hcp});
  DB.scores[nm] = Array(DB.settings.gameCount).fill(null);
  DB.dues[nm] = {};
  sortMembers();
  saveDB(); memberEditIdx=-1; renderMember(); T(`✅ ${nm} 추가`);
};
