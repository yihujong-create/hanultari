...
// 예기치 못한 오류를 화면에 드러냄(빈 화면 방지 및 원인 파악용)
window.addEventListener('error', function(ev){
  try{
    var msg='⚠️ 오류: '+((ev&&ev.message)||(ev&&ev.error&&ev.error.message)||'알 수 없음');
    var b=document.getElementById('syncBanner');
    if(b){ b.textContent=msg; b.classList.add('warn'); }
    console.error('전역 오류:', ev&&ev.error||ev&&ev.message);
  }catch(_){}
});
...
// ── 모임별 개인 평균(Aver)·핸디·팀 컨텍스트 ──────────────────────────────
// DB.meetAver/DB.meetHcp/DB.meetTeam = { 이름: 값 } 이면 "과거(불러온) 모임"을 보는 중 → 그 모임의 고정값 사용.
// null/미설정이면 "활성(현재) 모임" → 회원 기본값(m.aver/m.handicap/m.team) 사용.
// 이렇게 해서 새 모임용으로 값을 바꿔도 과거 모임의 결과(팀·벌금·시상·차이)가 바뀌지 않는다.
function averOf(m){
  if(!m) return 0;
  if(DB.meetAver && DB.meetAver[m.nm]!==undefined && DB.meetAver[m.nm]!==null) return DB.meetAver[m.nm];
  return m.aver||0;
}
// 모임별 팀 — 과거 모임이면 그 모임에 지정됐던 팀, 아니면 회원 기본 팀
function teamOf(m){
  if(!m) return '';
  if(DB.meetTeam && DB.meetTeam[m.nm]!==undefined && DB.meetTeam[m.nm]!==null) return DB.meetTeam[m.nm];
  return m.team||'';
}
// 과거 모임을 보는 중인지 (값이 그 모임 것으로 고정된 상태)
function isFrozenMeet(){ return !!(DB.meetAver && Object.keys(DB.meetAver).length); }
// 이력 스냅샷(memberSnap)에서 모임 Aver·핸디·팀 컨텍스트를 구성
function _setMeetAverFromSnap(h){
  if(h && Array.isArray(h.memberSnap) && h.memberSnap.length){
    ...
    const on = Object.keys(am).length>0;
    DB.meetAver = on ? am : null;   // 구버전 이력(스냅샷 없음)은 컨텍스트 비활성
    DB.meetHcp  = on ? hm : null;
    DB.meetTeam = on ? tm : null;
  }else{
    DB.meetAver = null;
    DB.meetHcp  = null;
    DB.meetTeam = null;
...
// 활성(새) 모임으로 전환 — 고정 컨텍스트 해제(현재 회원 기본값 사용)
function _clearMeetAver(){ DB.meetAver = null; DB.meetHcp = null; DB.meetTeam = null; }
...
_normalizeDB();   // Firebase/구버전 데이터의 점수 모양 보정 (배열 깨짐 방지)

// ── 데이터 정규화 ────────────────────────────────────
// Firebase 실시간 DB는 null이 섞인 배열을 객체({0:..,2:..})로 바꿔 저장한다.
// 그대로 두면 DB.scores[이름].filter 등에서 "filter is not a function" 오류가 난다.
// 불러올 때마다(로컬 로드·원격 동기화) 항상 정상 배열로 되돌린다.
function _toArrayMaybe(v){
  if(Array.isArray(v)) return v.slice();
  if(v && typeof v==='object'){
    const keys=Object.keys(v).filter(k=>/^\d+$/.test(k)).map(Number);
    if(keys.length){
      const len=Math.max(...keys)+1, arr=[];
      for(let i=0;i<len;i++) arr[i]=(v[i]===undefined?null:v[i]);
      return arr;
    }
    return [];
  }
...
function _cleanScoreArr(v, gc){
  let arr=_toArrayMaybe(v).map(x=>{
    if(x===null||x===undefined||x==='') return null;
    if(typeof x==='number') return x;
    const n=parseInt(x,10); return Number.isNaN(n)?null:n;
  });
...
function _normalizeDB(){
  ...
  const gc=(DB.settings&&DB.settings.gameCount)||3;
  // members 배열 보정
  if(!Array.isArray(DB.members)) DB.members=_toArrayMaybe(DB.members).filter(m=>m&&typeof m==='object');
  // 현재 점수 보정
  if(!DB.scores||typeof DB.scores!=='object') DB.scores={};
  Object.keys(DB.scores).forEach(nm=>{ DB.scores[nm]=_cleanScoreArr(DB.scores[nm], gc); });
  // 회원은 있는데 점수 키가 없으면 생성
  DB.members.forEach(m=>{ if(m&&!Array.isArray(DB.scores[m.nm])) DB.scores[m.nm]=Array(gc).fill(null); });
  // 이력 보정
  if(DB.scoreHistory!==undefined && !Array.isArray(DB.scoreHistory)) DB.scoreHistory=_toArrayMaybe(DB.scoreHistory).filter(h=>h&&typeof h==='object');
  if(Array.isArray(DB.scoreHistory)) DB.scoreHistory.forEach(h=>{
    if(!h||typeof h!=='object') return;
    const hgc=h.gameCount||gc;
    if(h.scores&&typeof h.scores==='object'){
      Object.keys(h.scores).forEach(nm=>{ h.scores[nm]=_cleanScoreArr(h.scores[nm], hgc); });
    }
    if(h.memberSnap && !Array.isArray(h.memberSnap)) h.memberSnap=_toArrayMaybe(h.memberSnap).filter(s=>s&&typeof s==='object');
    if(h.allCovers && !Array.isArray(h.allCovers)) h.allCovers=_toArrayMaybe(h.allCovers);
  });
  // 기타 배열 보정
  if(DB.allCovers!==undefined && !Array.isArray(DB.allCovers)) DB.allCovers=_toArrayMaybe(DB.allCovers);
}
window._normalizeDB=_normalizeDB;
...
function renderPage(id){
  const fn=({dash:renderDashboard,score:renderScore,award:renderAward,rank:renderRank,
  ...
  try{
    ...
    console.error('렌더 오류('+id+'):', err);
    const el=document.getElementById('pg-'+id);
    if(el) el.innerHTML=
      '<div style="padding:18px;color:#ffaaaa;font-size:13px;line-height:1.7">'
      +'⚠️ 화면을 표시하는 중 오류가 발생했습니다.<br>'
      +'<span style="color:var(--txt2)">페이지: '+id+'</span><br><br>'
      +'<code style="display:block;font-size:11px;color:var(--ora);background:var(--s2);border-radius:6px;padding:8px;word-break:break-all">'
      +String((err&&err.message)||err)+'</code><br>'
      +'<button onclick="location.reload()" style="background:#0d1e33;border:1px solid var(--blu);color:var(--blu);border-radius:7px;padding:8px 14px;font-size:12px;cursor:pointer;font-family:inherit;margin-right:6px">🔄 새로고침</button>'
      +'<button onclick="if(confirm(\'기기에 저장된 임시 데이터를 비우고 새로고침합니다. 클라우드(Firebase) 백업이 있으면 자동 복구됩니다.\')){try{localStorage.removeItem(\'hanultari_v2\')}catch(e){};location.reload();}" style="background:#2b0d0d;border:1px solid var(--red);color:var(--red);border-radius:7px;padding:8px 14px;font-size:12px;cursor:pointer;font-family:inherit">🗑 로컬 임시데이터 비우기</button>'
      +'</div>';
  }
}
...
function computeTeamStats(){
  ...
  DB.members.forEach(m=>{
    const tm=teamOf(m);
    if(!tStat[tm]) return;   // 팀 배정된 회원만 (A/B/C/D) — 동수도 팀이 있으면 포함
    const sc=(DB.scores[m.nm]||[]).filter(x=>x!==null&&x!==undefined);
    if(!sc.length) return;
    applyHcp(m.nm,sc).forEach(s=>{tStat[tm].t+=s;tStat[tm].n++;});
  });
  return tStat;
}
...
function renderDashboard(){
  ...
  const hasHistory=Array.isArray(DB.scoreHistory)&&DB.scoreHistory.length>0;
  ...
  if(hasHistory){
    ...
    DB.scoreHistory.forEach((h,hi)=>{
      if(!h || typeof h!=='object' || !h.date) return;   // 손상된 이력 항목 건너뜀
...
function renderScore(){
  ...
  if(scoreTab==='ind'){
    ...
    const teams={A:[],B:[],C:[],D:[],'':[]}; 
    DB.members.forEach(m=>{ const t=['A','B','C','D'].includes(teamOf(m))?teamOf(m):''; teams[t].push(m); });
    ['A','B','C','D',''].forEach(t=>{
...
function scoreCard(m,gc){
  ...
  const mteam=teamOf(m);                     // 이 모임에 적용되는 팀 (과거 모임이면 고정값)
  const tCls=teamCls(mteam);
  const roleH=m.role?`<span class="rb">${m.role}</span>`:'';
  ...
  let html=`<div class="sc-card" style="border-left:5px solid ${teamColor(mteam)};${isAbs?'opacity:.55;':''}">
    <div class="sc-hdr">
...
window.setAverByName=function(nm,v){
  ...
  if(isFrozenMeet()){
    ...
    if(ei>=0){
      ...
      else DB.scoreHistory[ei].memberSnap.push({nm,aver:val,handicap:getHcp(nm),team:teamOf(m),group:calcGrade(val),role:m.role});
    }
...
// 요청 — 점수입력 창에서 팀 지정 (모임별로 고정: 과거 모임은 그 모임만 변경)
window.setTeamByName=function(nm,t){
  const m=DB.members.find(x=>x.nm===nm); if(!m) return;
  const val=['A','B','C','D'].includes(t)?t:'';
  if(isFrozenMeet()){
    // 과거 모임만 수정 — 다른 모임·회원 기본값에 영향 없음
    if(!DB.meetTeam) DB.meetTeam={};
    DB.meetTeam[nm]=val;
    const ei=DB.scoreHistory?DB.scoreHistory.findIndex(x=>x.date===DB.meetDate):-1;
    if(ei>=0){
      if(!Array.isArray(DB.scoreHistory[ei].memberSnap)) DB.scoreHistory[ei].memberSnap=[];
      const snap=DB.scoreHistory[ei].memberSnap.find(s=>s.nm===nm);
      if(snap){ snap.team=val; }
      else DB.scoreHistory[ei].memberSnap.push({nm,aver:m.aver,handicap:getHcp(nm),team:val,group:calcGrade(m.aver),role:m.role});
    }
    saveDB(); renderScore(); T(`👥 ${nm} → ${val?val+'팀':'팀없음'} (이 모임만)`);
  }else{
    m.team=val;
    saveDB(); renderScore(); T(`👥 ${nm} → ${val?val+'팀':'팀없음'}`);
  }
};
...
window.setHcpByName=function(nm,v){
  ...
  if(isFrozenMeet()){
    ...
    if(ei>=0){
      ...
      else DB.scoreHistory[ei].memberSnap.push({nm,aver:m.aver,handicap:val,team:teamOf(m),group:calcGrade(m.aver),role:m.role});
...
window.archiveMeet=function(){
  ...
  const entry={
    ...
    // 회원 Aver·핸디 스냅샷
    memberSnap: DB.members.map(m=>({nm:m.nm,aver:averOf(m),handicap:getHcp(m.nm),team:teamOf(m),group:calcGrade(averOf(m)),role:m.role}))
  };
...
function histCurrent(){
  ...
  DB.members.forEach(m=>{
    ...
    const tc=teamColor(teamOf(m));
    html+=`<tr><td class="hnm">${m.nm}</td><td><span style="color:${tc};font-size:9px">${teamOf(m)||'-'}</span></td><td>${mav}</td>`;
